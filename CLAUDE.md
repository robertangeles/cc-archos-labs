...
## Before claiming a feature is unbuilt
**`wiki/state.md` is the source of truth for ship state.** It is auto-generated by `scripts/wiki-state.mjs` (run via the pre-commit hook on any change under `app/` or `components/`, or manually via `pnpm wiki:state`) and lists every shipped route, API endpoint, and component with its last-commit date.

Before claiming a route, API endpoint, or component does not exist:
1. Read `wiki/state.md`. If it is listed there, it is shipped — trust the register over any backlog claim.
2. If still uncertain, Glob the filesystem directly.
3. Only treat the backlog as authoritative for what is *planned*. Backlog files describe intent. The state register describes reality.
...
**There are TWO databases: a local DEV Postgres and a separate Render PROD Postgres. They are NOT the same DB.** Since 2026-06-15, `.env.local`'s `DATABASE_URL` points at the **local dev DB** (`archos_labs_dev`, host `127.0.0.1`, PG18, no SSL); production is the Render `archos_labs_pdb` (Singapore), kept commented in `.env.local` as `DATABASE_URL_RENDER_PROD`. **Anything run via `.env.local` — `db:migrate`, seeds, ad-hoc SQL, `scripts/db-apply.mjs` — hits DEV only.** PROD is migrated separately: a manual run of the idempotent `scripts/db-apply.mjs` against the PROD URL after a `pg_dump` backup (there is no migrate-on-deploy hook; `build`/`start` are vanilla). **Before claiming where data lives, check the `DATABASE_URL` host — do not trust prose (a `127.0.0.1` / no-SSL connection is DEV, full stop).** The pre-2026-06-15 "single database" framing is retired; do not reintroduce it.
...
- `wiki/raw-index/` — pointer pages to source content that lives elsewhere (Drive, Notion, private systems, large PDFs, client material). The wiki documents these without owning or relocating them.
- `wiki/raw/` — Karpathy Layer 1 sources checked into the repo verbatim (small, public, worth preserving). See `wiki/raw/README.md` for the placement rule.
