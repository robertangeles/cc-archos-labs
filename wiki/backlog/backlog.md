...
49. **Content calendar with scheduled publishing** — ✅ **SHIPPED 2026-06-14.** `scheduled_social_post` table, cron publisher with FOR UPDATE SKIP LOCKED (serialized per user+platform to prevent token refresh race), CRUD endpoints (list/create/cancel/reschedule/retry), publish modal extended with schedule toggle + datetime picker + per-platform time suggestions, scheduled posts list page with status badges and inline actions, disconnect warning + auto-cancel, publish confirmation email via Resend, upcoming posts widget, "Scheduled" tab in workspace nav. Operator step: configure Render Cron Job for `/api/cron/process-scheduled-social` every 60s.
...
- **▢ Per-org feature flags + admin toggle page** — the workspace-memory flags (`WORKSPACE_MEMORY_INGEST`, `WORKSPACE_TOOLS_ENABLED`, `WORKSPACE_SKILL_RECALL`) are **global** `process.env` booleans (`isWorkspaceMemoryEnabled()` etc. take no `orgId`), so flipping one enables the feature for **every** org at once. The plan's "enable one org, watch, expand" rollout is therefore **not achievable today** — data isolation is per-org and solid, but activation is all-or-nothing. Two-part fix: **(B)** a per-org flag (e.g. `organisation.workspace_memory_enabled` column) that `isWorkspaceMemoryEnabled(orgId)` reads, enabling a genuine staged rollout; **(A)** a small admin toggle page so features flip from the UI without a Render redeploy (sits on top of B). Touches schema + the three enable checks + a new admin surface → run plan mode first. **Verify:** enabling org A's flag surfaces workspace recall for org A only; org B stays dark until its own flag is set.
- **▢ CSRF check missing on `DELETE /api/brain/memories`** — neither the private-tier delete nor the workspace-tier delete (`?scope=workspace`, added in PR #204) calls `assertSameOriginRequest`, unlike the org-context mutation guard used elsewhere. Pre-existing pattern, surfaced by the PR #204 review; not introduced by it. Layered mitigation exists today (per-user rate limit, session-based auth, `SameSite: lax` on the auth cookie), so severity is low — a cross-site request can't ride the cookie under `lax` for a non-top-level POST/DELETE. Fix: add the same-origin assertion to both DELETE branches. **Verify:** a cross-origin `DELETE` (mismatched `Origin`) returns 403; same-origin delete still works.
