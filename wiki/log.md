...
## 2026-06-22 — Fix: Brain recall silently failing on GBrain cold starts ("Metis doesn't remember me")

- **Report:** "the Metis chat space does not remember me now." Investigated as a debugging task (not data loss). Tested on the live site (PROD), signed in as `trebor.selegna@outlook.com`.
- **Root cause (confirmed by forensics, not guessed):** memory recall (`lib/brain/recall.ts`) makes two sequential GBrain calls (token + vector query), each on a 5s timeout, and swallows every failure (returns empty, chat proceeds with no memory). GBrain cold-starts on Render; the same query measured **~1.5s warm but >5s cold**, so the first recall after idle blew the 5s budget → zero memories → generic "Do you remember me?" answer. Intermittent ("now").
- **Ruled out with evidence:** PROD DB schema (rolled-back insert succeeded), persistence (real `chatService` create→save→read→delete worked; live convo "Hello There" persisted 4 msgs), credentials/config (token + config resolved), auth (brain `last_active` updated live), stale deploy (`/search` 200, chat API 401), wrong DB (deployment-architecture.md confirms live = the probed `archos_labs_pdb`). The user's 5 memories existed and recalled fine when warm.
...
- **✅ RESOLVED (2026-07-01):** the 20:00 scheduled post published successfully and recorded `published` — both fixes (`202606` version + `x-restli-id` header read) confirmed live in PROD. LinkedIn auto-posting is working end-to-end.
- **Disconnect item — closed unpursued:** the DB-only disconnect failure was not chased to root cause; publishing (the actual goal) is fixed, so it was deprioritised. Reopen if it recurs — capture the `POST /api/social/linkedin/disconnect` status code + body first.

## 2026-07-06 — Workflow step regeneration (shipped, live in PROD)

Built and shipped the per-step **Regenerate** feature end to end via the full plan pipeline: `/plan-ceo-review` (SELECTIVE EXPANSION — accepted E1 make-permanent, E3 model override, E4 live/concurrency; skipped a compare view) → `/plan-eng-review` → `/plan-design-review` → three staged PRs, each smoke-tested live and independently reviewed by `pr-reviewer`.

- **PR #179** — extracted the shared `executeStep` primitive from the executor's two duplicated loop bodies (pure, behavior-preserving) + first executor tests. The eng-review outside voice reversed an initial "unify both loops into one generator" plan: the eager `executeWorkflow` path is dead for the UI, so a per-step `executeStep` extraction is the safe seam.
- **PR #180** — the core: SSE regenerate route, server-side context rebuild (client never supplies context), 409 drift guard, overwrite-only-on-success `amendRun` (evict-race rows-affected guard, SSE-cancel persist, failed-resume append), extended `StepResult` provenance/stale fields (JSONB, no migration), and the `StepResultCard` UI. Design's load-bearing call: a failed regenerate shows a dismissible notice on the normal surface, never the red failed-step body over a kept-good output.
- **PR #181** — E1 make-permanent (feedback → step prompt via a surgical owner-scoped `appendStepPrompt`, NOT the delete-all workflow PUT), E3 per-run model picker, and prompt-injection hardening (prior output fenced as quoted data with a per-call random marker). `pr-reviewer` caught a real stale-`confirmPermanent` UI bug + missing owner-scoping across the PRs.
