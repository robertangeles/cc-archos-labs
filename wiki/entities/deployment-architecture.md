...
- **PROD migration state: `0037_post_reviewed_by_human_at.sql`, applied 2026-07-29.** Backup `~/archos_prod_backup_20260729-071238.dump` (26M, pg_restore-verified) taken first; the run skipped 11 prior migrations and applied 0037 alone (2 statements). Column landed as `timestamp with time zone` nullable, comment applied, 319 existing posts all NULL — no byline changed. Local pg_dump and the Render server were both 18.4. Before this run, `__drizzle_applied` ran through `0036_content_plan_item.sql`. `0035_workspace_memory.sql` **is** applied — earlier wiki pages recorded `0031`-`0034` and `0036` but never mentioned `0035`, and that silence read as a gap during the PR #223 pre-flight. It was not one. **When you migrate PROD, record the migration number here**, so the next person does not have to re-derive it from a live connection.
