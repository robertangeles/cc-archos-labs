...
**A host in `script-src` or `img-src` is not thereby allowed to `fetch`.** Review of this change found two more instances of the same mistake, both invisible for the same reason as the first. `fbevents.js` sends its primary event via an `Image()` GET (`img-src`, listed) but falls back to `sendBeacon`/`fetch` against `www.facebook.com/tr/` on unload (`connect-src`, not listed). Turnstile's `api.js` issues tokens through its iframe via `postMessage` (ungoverned) but also fetches `/cdn-cgi/challenge-platform/...` from the top-level page. Third-party tags routinely use several transports; allowlist per transport, not per vendor.

**Some resources are injected by infrastructure you do not control, so they are in no codebase to read.** Immediately after the flip, production made a GET to `challenges.cloudflare.com/cdn-cgi/challenge-platform/h/b/pat/...`. That path is in no application code, and the served HTML of `/`, `/login` and `/contact` reference it zero times — Cloudflare injects it at the edge, after parse, on proxied pages. It is Cloudflare's own bot-management telemetry and it is live with Turnstile switched off. A fourth blind spot on top of the three above: not in the source, not in the markup, not in the reports, and *not authored by us at all*. Anything behind a proxy or CDN can have resources added to its pages by that layer.

[Inference] It stayed out of two days of Report-Only data because it fires for bot-suspicious clients and that traffic was human; a headless browser triggered it on the first request. Not directly confirmed. What is confirmed: it fires, we do not emit it, and `connect-src` governs it.

**Justify an allowlist entry by the mechanism you verified, not the one that sounds right.** This entry was first defended as Turnstile's clearance-redemption flow, which was plausible, Zone-gated, and wrong. Had that stood, the next reader would have seen "Turnstile is off" and deleted a live dependency. A wrong reason attached to a correct line is worse than no comment, because it reads as permission to remove it.

**Inert today is the most dangerous state.** Neither of those two could appear in the violation stream at all — Meta Pixel has no configured ID and Turnstile defaults off. A feature that is switched off cannot generate the evidence that would protect it, so it gets allowlisted from reading the vendor's actual script or not at all. Turnstile compounds this: the widget tolerates a missing script silently, so a blocked bot gate on auth and booking produces no error anywhere.
