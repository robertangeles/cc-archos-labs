  ...
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-[760px] flex-col">
        ...
        <h2 className="mt-4 text-2xl font-semibold leading-[1.2] tracking-[-0.015em] text-fg md:text-[36px]">
          {question.text}
        </h2>
            ...
            return (
              <button
                ...
                className={`group flex items-start gap-x-5 rounded-lg border px-5 py-4 text-left transition-all duration-150 md:px-6 md:py-5 ${
                  isSelected
                    ? "border-accent bg-accent/10 shadow-[0_0_0_1px_rgba(59,130,246,0.4)]"
                    : isLocked
                      ? "border-rule/60 bg-surface/40 opacity-60"
                      : "border-rule bg-surface hover:border-accent/50 hover:bg-surface/80"
                }`}
              >
                <span
                  className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md border font-mono text-[12px] font-semibold transition-colors duration-150 ${
                    isSelected
                      ...
                      : "border-rule text-muted group-hover:border-accent/60 group-hover:text-fg"
                  }`}
                >
                  {opt.code}
                </span>
