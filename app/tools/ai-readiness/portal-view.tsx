  ...
  return (
    <main className="flex flex-1 flex-col bg-canvas">
      <section className="mx-auto w-full max-w-[840px] px-6 pt-24 pb-32 md:px-12 md:pt-32">
        ...
        <h1 className="mt-4 text-3xl font-semibold leading-[1.1] tracking-[-0.02em] text-fg md:text-[40px]">
    ...
    <div className="mt-12">
      ...
      <ul className="mt-5 flex flex-col gap-y-3">
        {reports.map((r) => (
          <li
            key={r.sessionId}
            className="rounded-md border border-rule bg-surface px-5 py-5 transition-colors duration-150 hover:border-accent/50 md:px-6 md:py-6"
          >
            <Link
            ...
            >
              <div className="flex items-baseline gap-x-5">
                <span className="font-mono text-2xl font-semibold leading-none text-fg md:text-[32px]">
                  {r.totalScore}
                </span>
...
function NoReportsBlock() {
  return (
    <div className="mt-12 rounded-md border border-rule bg-surface px-6 py-6">
...
function RetakeAvailable() {
  return (
    <>
      ...
      <p className="mt-3 max-w-[600px] text-base leading-[1.6] text-muted">
...
function RetakeLocked({ unlocksAt }: { unlocksAt: Date | null }) {
  ...
  return (
    <>
      ...
      <p className="mt-3 max-w-[640px] text-base leading-[1.6] text-muted">
      ...
      <div className="mt-8 flex flex-wrap items-center gap-x-4 gap-y-2">
        <span className="inline-flex items-center rounded-md border border-rule bg-surface px-5 py-3 text-base font-medium text-muted">
          Retake locked
        </span>
        <Link
          href="/contact"
