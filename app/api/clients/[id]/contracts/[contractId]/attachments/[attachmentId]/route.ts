...
// DELETE /api/clients/:id/contracts/:contractId/attachments/:attachmentId —
// remove an attachment. Owner|admin. 404 when the attachment is not in the
// caller's org, so cross-org ids leak nothing.
export async function DELETE(
  request: Request,
  {
    params,
  }: {
    params: Promise<{ id: string; contractId: string; attachmentId: string }>;
  },
) {
  try {
