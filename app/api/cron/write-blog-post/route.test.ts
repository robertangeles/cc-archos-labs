...
vi.mock("../../../../lib/blog-agent/run", () => ({ runOnce: () => runOnceMock() }));
...
beforeEach(() => {
  ...
  runOnceMock.mockResolvedValue({ outcome: "idle" });
...
describe("auth", () => {
  ...
  it("200s with the correct token", async () => {
    ...
    expect(runOnceMock).toHaveBeenCalledOnce();
  });
});
...
describe("run outcomes", () => {
  ...
  it("reports not-due without doing work", async () => {
    runOnceMock.mockResolvedValue({ outcome: "not-due" });
  ...
  it("503s but does not throw when the agent itself throws", async () => {
    runOnceMock.mockRejectedValue(new Error("boom"));
    const res = await POST(req(`Bearer ${SECRET}`));
    expect(res.status).toBe(503);
